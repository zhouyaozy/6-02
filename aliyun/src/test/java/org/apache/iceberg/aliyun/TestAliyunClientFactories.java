...
public class TestAliyunClientFactories {
  ...
  @Test
  @SetEnvironmentVariable(key = "ALIBABA_CLOUD_OIDC_PROVIDER_ARN", value = "")
  public void testIsRrsaEnvironmentAvailableWithPartialEnvVarsOnlyProvider() {
    AliyunClientFactories.DefaultAliyunClientFactory factory =
        new AliyunClientFactories.DefaultAliyunClientFactory();

    assertThat(factory.isRrsaEnvironmentAvailable())
        .as("RRSA should not be available with only one env var set")
        .isFalse();
  }

  @Test
  @SetEnvironmentVariable(key = "ALIBABA_CLOUD_OIDC_PROVIDER_ARN", value = "valid")
  @SetEnvironmentVariable(key = "ALIBABA_CLOUD_ROLE_ARN", value = "valid")
  public void testIsRrsaEnvironmentAvailableWithPartialEnvVars() {
    AliyunClientFactories.DefaultAliyunClientFactory factory =
        new AliyunClientFactories.DefaultAliyunClientFactory();

    assertThat(factory.isRrsaEnvironmentAvailable())
        .as("RRSA should not be available with only two env vars set")
        .isFalse();
  }

  @Test
  public void testNewOSSClientBeforeInitialization() {
    AliyunClientFactories.DefaultAliyunClientFactory factory =
        new AliyunClientFactories.DefaultAliyunClientFactory();
    assertThrows(
        NullPointerException.class,
        factory::newOSSClient,
        "Should throw exception when creating OSS client before initialization");
  }
  ...
  public void testNewOSSClientWithAccessKey() {
    ...
    assertThat(client).as("OSS client should be created with access key").isNotNull();
    client.shutdown();
  }
  ...
  public void testNewOSSClientWithSecurityToken() {
    ...
    OSS client = factory.newOSSClient();
    assertThat(client).as("OSS client should be created with security token").isNotNull();
    client.shutdown();
  }
